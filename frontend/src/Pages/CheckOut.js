...
const CheckOut = () => {
  ...
  if (!Cart) return <div className="mt-48 ml-44"><CircleLoader color="black" size={25} loading={true} /></div>;
  ...
  if (Cart?.length !== 0) {
    ...
    toShow = <div className="flex flex-wrap gap-3 md:gap-6">{
      data?.map((product) => {
  ...
  return (
    <>
      ...
      <div className="mt-[5.5rem] px-2 relative">
        <div className=" text-xl font-semibold tracking-wider text-yellow-500">Checkout</div>
        {Cart?.length !== 0 ?
          <div>
            {toShow}
            <p className="text-lg font-semibold">Total Price : <span className="text-md font-medium ml-2"> ₹{checkOutData.totalPrice} </span></p>
            <button
            ...
            <button className="ml-4 border border-black py-2 px-4 hover:bg-yellow-500 hover:border-yellow-500 hover:text-white"><Link to="/product">Products</Link></button>
          </div>
          :
          <>
            <div className="mt-[22vh] mx-auto w-fit"><img src={EmptyCart} alt="" /></div>
            <div className=' border border-black py-1 px-3 w-20 mx-auto text-sm rounded hover:bg-yellow-500 hover:border-yellow-500 hover:text-white '><Link to="/product">Products</Link></div>
          </>
        }
      </div>
    </>
