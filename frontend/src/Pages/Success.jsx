...
const setProducts = async () => {
  const response = await axios.get('https://aais-kitchen-backend.onrender.com/api/product');
...
const getOrder = async (session_id) => {
  ...
  if (response?.data?.session?.payment_status === "paid") {
    ...
    if (purchased.data) {
      ...
      const result = await axios.patch("https://aais-kitchen-backend.onrender.com/api/users/cart", {
        userId: user.id,
        productId: ''
      })
  ...
  return (
    <div>
      ...
      <div className="ml-2 md:flex md:items-center md:justify-center md:flex-col">
        <div className="flex items-center mt-20 text-sm md:text-md"><span className="text-yellow-500 text-2xl md:text-3xl font-semibold">Success</span>, your order is placed! <MdVerified color="green" size={25} className="ml-2" /></div>
        {invoice ? (
        <div className="mt-4 text-sm md:text-md md:text-center">
          ...
          <div className="flex items-center gap-4 mt-8 md:justify-center">
            <div className="px-2 py-1 bg-yellow-500 text-white rounded text-sm md:text-md cursor-pointer"><Link to="/product"> Order More</Link></div>
            <div className="px-2 py-1 bg-yellow-500 text-white rounded text-sm md:text-md cursor-pointer"><Link to="/orders"> My Orders</Link></div>
          </div>

          <div className="mx-auto mt-8"><img src={foodDelivery} alt='' className="w-72 h-72 mx-auto" /></div>
        </div>
        ) : (
