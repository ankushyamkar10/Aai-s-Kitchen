...
const Favourites = () => {
  ...
  const toShow = favAndCart.favList.length <= 0 ? <div className="flex flex-col items-center mt-40">
    <img src={EmptyBox} alt="empty box" className="w-56" />
    <div className="text-sm font-semibold mb-4 -mt-4">Add dishes to favourites!</div>
    <Link to="/product">
  ...
  </div> : <div className="flex flex-wrap items-center justify-start gap-[2vw] ml-2">
    {data?.map((product) => {
      return (
        <div key={product._id}  >
  ...
  // if (favAndCart.favList.length <= 0) {
  //   toShow = <div className="flex flex-col items-center">
  //     <img src={EmptyBox} alt="empty box" className="w-60" />
  //     <div className="text-sm font-semibold mb-6">Add some dishes to favourites!</div>
  //     <Link to="/product">
  //       <div className={`border border-black py-1 px-3 w-fit mx-auto text-sm rounded hover:bg-yellow-500 hover:border-yellow-500 hover:text-white `}> Add + </div>
  //     </Link>
  //   </div>
  // }
  // else {
  //   toShow = <div className="flex flex-wrap items-center justify-start gap-[2vw] ml-2">
  //     {data?.map((product) => {
  //       return (
  //         <div key={product._id}  >
  //           <ProductWrapper product={product} favAndCart={favAndCart} setFavAndCart={setFavAndCart} />
