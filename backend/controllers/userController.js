...
const getMe = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ _id, name, email, favourites, cart, purchased, token });
});
...
const getUserOtherData = asyncHandler(async (req, res) => {
  ...
  if (req.url === "/cart") res.status(200).json(user.cart);
  else if (req.url === "/favourites") res.status(200).json(user.favourites);
  else if (req.url === "/purchased") res.status(200).json(user.purchased);
});
